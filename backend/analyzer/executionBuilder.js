...
function buildExecution(program) {
  ...
  const next = (from, type = "normal") => {
    const edge = edges.find(
      (e) => e.from === from && e.type === type
    );
    return edge?.to || "exit";
  };

  // START is a UI/runtime event, not a parsed statement.
  push({
    node: "start",
    line: null,
    type: "start",
  ...
  while (current !== "exit" && guard++ < MAX_STEPS) {
    ...
    if (s.type === "else") {
      push({
        ...
        type: "else",
        variables: { ...variables },
        explanation: "The ELSE branch is executing.",
      });
...
function evaluateCondition(expression, variables) {
  ...
  if (condition.includes("&&")) {
    return condition
      .split("&&")
      .every((part) =>
        evaluateCondition(part, variables)
      );
  }
  ...
  for (const op of [
    ...
    const left = evaluateValue(
      condition.slice(0, pos),
      variables
    );

    const right = evaluateValue(
      condition.slice(pos + op.length),
      variables
    );
...
function executeForInitializer(init, variables) {
  ...
  if (declaration) {
    variables[declaration[1]] = evaluateValue(
      declaration[2] ?? "0",
      variables
    );
    return;
  ...
  const assignment = init.match(
    /^([A-Za-z_]\w*)\s*=\s*(.*)$/
  );

  if (assignment) {
    variables[assignment[1]] = evaluateValue(
      assignment[2],
      variables
    );
  }
...
function executeForUpdate(update, variables) {
  ...
  const clean = String(update)
    .trim()
    .replace(/;$/, "");

  const increment = clean.match(
    /^(?:\+\+([A-Za-z_]\w*)|([A-Za-z_]\w*)\+\+)$|^(?:--([A-Za-z_]\w*)|([A-Za-z_]\w*)--)$|^([A-Za-z_]\w*)\s*(\+=|-=)\s*(.+)$/
  );
