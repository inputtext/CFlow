...
function ExecutionStory() {
  ...
  return (
    <section className="execution" ref={ref}>
      ...
      <div className="execution__stage"><div className="execution-code">{codeLines.map((line, index) => <div className="exec-line" key={line}><span>{index + 1}</span><code>{line}</code></div>)}</div><div className="execution-memory">{[['x', '10'], ['y', '20'], ['sum', '30']].map(([name, value]) => <div className="memory-card" key={name}><small>{name}</small><strong>{value}</strong></div>)}</div><div className="exec-result">OUTPUT <b>30</b></div></div>
    </section>
  )
}
...
function ProductPreview() {
  ...
  return <section className="scene scene--preview" ref={ref}><div className="section-kicker">07 / THE PRODUCT</div><div className="preview-copy"><h2 className="display display--medium">THE REAL<br /><span>WORKSPACE.</span></h2><p>Write C or C++. Run it. Then follow every meaningful state as your program moves.</p><div className="preview-tags"><span>CODE</span><span>MEMORY</span><span>FLOW GRAPH</span></div></div><div className="preview-window"><div className="preview-code"><span>01</span> int main() {'{'}<br /><span>02</span> &nbsp;&nbsp;int x = 10;<br /><span>03</span> &nbsp;&nbsp;return x;<br /><span>04</span> {'}'}</div><div className="preview-flow"><b>main()</b><i>↓</i><b>x = 10</b><i>↓</i><strong>RETURN 10</strong></div><div className="preview-window__bar">FLOW STATE <span>● READY</span></div></div></section>
}
...
export default function LandingPage() {
  ...
  return <main className="landing" ref={page}>
