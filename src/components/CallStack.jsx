...
function CallStack() {
  ...
  // The backend can later publish real function frames without coupling the
  // visual component to the parser/execution engine. Until then, main() is a
  // truthful root frame populated from the existing Memory panel.
  useEffect(() => {
    const syncFromMemory = () => {
      const nextFrames = readMemoryFrames();
      if (nextFrames.length) {
        setFrames((current) => current.length ? nextFrames : nextFrames);
      }
    };
    ...
    const handleStackEvent = (event) => {
      ...
      if (nextFrames.length) setOpen(true);
    };

    syncFromMemory();
    const observer = new MutationObserver(syncFromMemory);
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span className="font-mono text-[8px] font-bold uppercase tracking-wider opacity-45">
                    #{index + 1}
                  </span>
                </div>
